...
`line_config.yaml` is used to store line specific environment variables. Required variables are are `DOCKER_PLATFORM`, `INSPECTION_VERSION`, `FACTORY_CLIENT_ID`, `GADGET_DEVICE_ID`, `VAULT_CREDENTIAL`, and `GADGET_APP_TITLE`. `SUBFOLDER` indicates to location of the docker compose file. If left blank it will use `./default`.
...
- **`INSPECTION_VERSION`**: Optional version tag for images created using the **build** command. Defaults to a unique identifier based on machine name.
- **`LATEST`**:  Optional variable to tag images being pushed to gcp as latest
...
### 3. `build`
Build Docker images using the resolved Docker Compose file. Optional variables to skip build cache or specify version tag

**Usage:**
```bash
make build
make build NO-CACHE=y
make build INSPECTION_VERSION=v1.2
```
...
### 5. `push`
Push Docker images build locally to GCP. Optional variables:

- ``INSPECTION_VERSION``: Which version of the images to push. Defaults to use the value in custom.env.

- ``LATEST``: Tells the makefile to additionally tag the image as latest. Defaults to false.

**Usage:**
```bash
make push
make push INSPECTION_VERSION=v1.2 LATEST=true
```
...
### 9. `auth`
Authenticates the device with the GCP artifact repo for this project. Prompts the user to enter access token generated by **Generate Access Token** GitHub action.
...
Docker images are pulled from two locations. Gadget platform images are pulled from a docker repo hosted by LMI Technologies. The edge device will need to be given access to that repo by a member of the LMI AI Solutions team. 

Custom images built for specific applications are stored in a GCP artifact repo. To pull those images, generate a temporary access token using the projects GitHub action `Generate GCP Access Token`. The token will be printed during the `Generate access token` step. On the device, `make auth` will prompt the user to enter the access token. This will give the device permission to pull from the repo for the next hour.

### Image Versioning

The image version used is defined in two locations. In the `.env` file, the `PACKAGE_VER` variable defined the version for the Gadget platform containers. In the `line_config.yaml`, the `INSPECTION_VERSION` variable defines the version for the custom images.

## Creating Image Versions

To generate new versions of the custom docker images use the GitHub action `Build and Push Docker Images`. The action will give the user the option to specify which line to build, it defaults to `default`. Running the action will build the image for the architecture specified for that line and push them to the corresponding GCP artifact repo. The images will be tagged with a run number and `latest`. The version tag can be found in the workflow logs.
